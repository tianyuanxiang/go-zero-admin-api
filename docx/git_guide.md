...
本文档涵盖 `.gitignore` 语法详解、日常开发工作流、分支管理、敏感信息保护等内容，适用于本项目（go-zero-admin-api）的日常协作开发。
...
- [二、敏感信息保护](#二敏感信息保护)
  ...
  - [2.3 已经被 Git 跟踪的敏感文件如何移除](#23-已经被-git-跟踪的敏感文件如何移除)
...
- [五、日常开发工作流（dev 分支）](#五日常开发工作流dev-分支)
  - [5.1 标准开发流程](#51-标准开发流程)
  - [5.2 提交信息规范](#52-提交信息规范)
- [六、合并 dev 到 master（发布流程）](#六合并-dev-到-master发布流程)
  - [6.1 方式一：命令行合并](#61-方式一命令行合并)
  - [6.2 方式二：GitHub Pull Request](#62-方式二github-pull-request)
- [七、远程仓库操作](#七远程仓库操作)
- [八、冲突解决](#八冲突解决)
- [九、标签管理（版本发布）](#九标签管理版本发布)
- [十、实用技巧](#十实用技巧)
...
| 语法 | 含义 | 示例 |
|------|------|------|
| `文件名` | 排除所有同名文件（不论在哪个目录下） | `config.yaml` 会排除项目中所有叫 config.yaml 的文件 |
| `目录名/` | 排除整个目录（末尾加 `/`） | `uploads/` 会排除 uploads 目录及其所有内容 |
| `/文件名` | 仅排除项目根目录下的该文件 | `/config.yaml` 只排除根目录的 config.yaml，不影响子目录的 |
| `路径/文件名` | 排除指定路径下的文件 | `etc/config.yaml` 只排除 etc 目录下的 config.yaml |
| `!文件名` | 取反，表示"不排除"（强制包含） | `!important.yaml` 即使上面的规则排除了 *.yaml，这个文件也会被保留 |
| `# 注释` | 以 `#` 开头的行是注释，Git 会忽略 | `# 这是注释` |
| 空行 | 空行会被忽略，用于分组增加可读性 | |
...
| 通配符 | 含义 | 示例 | 匹配结果 |
|--------|------|------|----------|
| `*` | 匹配任意多个字符（不含 `/`） | `*.log` | app.log、error.log、debug.log |
| `**` | 匹配任意多层目录 | `**/logs` | logs、src/logs、src/app/logs |
| `?` | 匹配单个任意字符 | `file?.txt` | file1.txt、fileA.txt，但不匹配 file12.txt |
| `[abc]` | 匹配方括号内的任意一个字符 | `file[0-9].txt` | file0.txt 到 file9.txt |
| `[!abc]` | 匹配不在方括号内的任意一个字符 | `file[!0-9].txt` | fileA.txt，但不匹配 file1.txt |
...
*.log                  # 排除所有 .log 文件：app.log, error.log, tmp/debug.log
*.exe                  # 排除所有 .exe 文件：main.exe, tools/build.exe
temp_*                 # 排除以 temp_ 开头的文件：temp_data.csv, temp_cache

# --- 双星号 ** ：匹配任意多层目录 ---

**/build               # 排除任意深度的 build 目录：build/、src/build/、a/b/build/
logs/**                # 排除 logs 目录下的所有内容（但 logs 目录本身会保留）
**/test/**/*.tmp       # 排除任意 test 目录下任意深度的 .tmp 文件

# --- 问号 ? ：匹配单个字符 ---

config?.yaml           # 匹配 config1.yaml、configA.yaml，不匹配 config12.yaml
?.txt                  # 匹配 a.txt、1.txt，不匹配 ab.txt

# --- 方括号 [] ：匹配指定范围的单个字符 ---

log[0-9].txt           # 匹配 log0.txt 到 log9.txt
...
下面是一个 Go 项目的 `.gitignore` 完整示例，每行都有注释：
...
如果文件已经被 Git 跟踪过（之前提交过），`.gitignore` 无法直接生效。需要先从 Git 缓存中移除：
...
# 但保留 logs 目录下的 .gitkeep 文件（用于保持空目录）
!logs/.gitkeep
```
...
# 表示 .gitignore 第 30 行的规则排除了这个文件

# 如果没有输出，说明该文件没有被任何规则排除
```
...
| 文件 | 包含内容 | 是否已排除 |
|------|----------|-----------|
| `etc/config.yaml` | MySQL密码、Redis密码、JWT密钥、服务器IP | 已排除 |
...
# 3. 编辑 config.yaml，填入真实的数据库密码、Redis密码等
# 用你喜欢的编辑器打开：
...
假设将来项目新增了 `.env` 文件或 `certs/server.key` 密钥文件：

```bash
# 第一步：在 .gitignore 中添加规则
echo ".env" >> .gitignore
echo "certs/*.key" >> .gitignore

# 第二步：提交 .gitignore 的变更
git add .gitignore
git commit -m "chore: add .env and key files to gitignore"
git push origin dev
```

### 2.3 已经被 Git 跟踪的敏感文件如何移除

如果你不小心把敏感文件提交到了 Git，需要这样处理：

```bash
# 场景：etc/config.yaml 已经被提交过了，现在要从 Git 中移除

# 第一步：在 .gitignore 中添加排除规则
# 确保 .gitignore 中已有 etc/config.yaml

# 第二步：从 Git 跟踪中移除（--cached 表示只删除 Git 记录，不删除本地文件）
git rm --cached etc/config.yaml
...
# 此时：
# - 本地 etc/config.yaml 仍然存在，不影响你本地运行
# - 远程仓库中 etc/config.yaml 被删除
# - 后续提交不会再包含这个文件
```
...
#
# Untracked files:                  <-- 新文件，还没被 Git 跟踪
...
```bash
# --- 场景1：修改了文件但还没 git add，想恢复原样 ---
git checkout -- internal/logic/auth/login_logic.go
# 注意：这会丢失所有未暂存的修改，无法恢复

# --- 场景2：已经 git add 了，想取消暂存但保留修改 ---
git reset HEAD internal/logic/auth/login_logic.go
# 文件回到"已修改但未暂存"状态，修改内容还在

# --- 场景3：已经 git commit 了，想撤销这次提交但保留代码修改 ---
git reset --soft HEAD~1
# HEAD~1 表示回退1个提交
# --soft 表示代码修改保留在暂存区，可以重新提交

# --- 场景4：已经 git commit 了，想完全撤销（代码也回退） ---
git reset --hard HEAD~1
# 警告：--hard 会丢失代码修改，无法恢复，谨慎使用！

# --- 场景5：已经 push 到远程了，想撤销 ---
# 不要用 reset，用 revert 创建一个"反向提交"
git revert HEAD
...
| 分支 | 用途 | 谁来操作 |
|------|------|---------|
| `master` | 稳定版本，用于生产部署 | 只通过 dev 合并过来 |
| `dev` | 日常开发 | 开发人员直接提交 |
| `feature/xxx`（可选） | 开发大功能时从 dev 拉出 | 开发完成后合并回 dev |
...
# ============================================
# 第二步：切换到 master 分支，拉取最新代码
# ============================================
git checkout master
git pull origin master

# ============================================
# 第三步：将 dev 合并到 master
# ============================================
git merge dev
# 如果没有冲突，会自动完成合并
...
场景：你正在 dev 上开发，突然需要切到 master 修复一个紧急 Bug。
...
# 注意：只能修改最后一次提交，且该提交还没有 push 到远程

# 场景：刚提交完发现漏了一个文件
git add internal/handler/auth/login_handler.go
git commit --amend --no-edit
# --no-edit 表示不修改提交信息，只补充文件
```
...
git co dev      # 等同于 git checkout dev
...
Git 不会跟踪空目录。如果你想保留一个空目录（比如 `uploads/`），在里面放一个空文件：

```bash
# 在空目录中创建 .gitkeep 文件
touch uploads/.gitkeep

# .gitignore 中保留 .gitkeep
# uploads/*
# !uploads/.gitkeep
```
...
| 暂存修改 | `git stash` |
| 恢复暂存 | `git stash pop` |
| 检查忽略 | `git check-ignore -v 文件名` |
| 取消暂存 | `git reset HEAD 文件名` |

```
徐州：戏马台、博物馆、云龙湖、云龙山；烧烤不清楚，人多
沛县：庆景冷面(老店，人多)，徐庄矿老四冷面(老店，不在县城，比较偏，但好吃)，铁西老二羊杂冷面(味道可以，不出名)，老味道冷面(歌风佳苑北面的店)、沛风羊肉，鹿串烧烤、广阔烧烤、早上可以喝一杯撒汤（温州商贸城庆云撒汤）
