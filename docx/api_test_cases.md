...
**用例1: 默认分页** -- 不传参数，使用默认 page=1, pageSize=20
```
GET /api/system/user
```
...
> 说明: 角色的基础 CRUD 和登录登出 BOSS 已写好，此处仅补充权限分配接口的测试用例
...
18. **角色权限分配** -- 测试菜单祖先链补全
19. **日志列表** -- 前面操作已经产生了日志
20. **删除类测试** -- 按 字典数据 > 字典类型 > 接口 > 菜单 > 用户 的顺序删除测试数据
21. **清空日志** -- 最后清理
22. **修改密码** -- 修改当前用户密码
23. **刷新令牌** -- 测试令牌刷新
24. **登出** -- 最后执行
